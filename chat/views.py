...
def specialist_rooms(request):
    ...
    if user_profile.user_type == 1:  # 1 corresponds to 'Specialist'
        ...
        return redirect('home')  # Change to appropriate redirect for non-specialists
...
def take_room(request, room_name):
    ...
    if user_profile.user_type == 1:  # 1 corresponds to 'Specialist'
        room = ChatRoom.objects.filter(name=room_name, specialist__isnull=True).first()
    ...
    # If the user is not a specialist or room is already taken/invalid, redirect to the list of rooms
    return redirect('chat:specialist_rooms')
